...
func (s *Suite) TestJaeger_and_prometheus() {
	...
	r.Run(`kubectl apply -k https://github.com/d-uzlov/deployments-k8s/examples/observability/jaeger-and-prometheus?ref=63d2effb4e93b0238bd80c5af9bd899b1245b6ba`)
	r.Run(`kubectl wait -n observability --timeout=1m --for=condition=ready pod -l app=opentelemetry`)
	r.Run(`kubectl apply -k https://github.com/d-uzlov/deployments-k8s/examples/observability/jaeger-and-prometheus/nsm-system?ref=63d2effb4e93b0238bd80c5af9bd899b1245b6ba`)
	r.Run(`WH=$(kubectl get pods -l app=admission-webhook-k8s -n nsm-system --template '{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}')` + "\n" + `kubectl wait --for=condition=ready --timeout=1m pod ${WH} -n nsm-system`)
	r.Run(`kubectl apply -k https://github.com/d-uzlov/deployments-k8s/examples/observability/jaeger-and-prometheus/example?ref=63d2effb4e93b0238bd80c5af9bd899b1245b6ba`)
