...
func (s *Suite) TestJaeger_and_prometheus() {
	...
	r.Run(`kubectl apply -k https://github.com/d-uzlov/deployments-k8s/examples/observability/jaeger-and-prometheus?ref=f3d8e97881a29f22b150bfcd6233647c5f96ee68`)
	r.Run(`kubectl wait -n observability --timeout=1m --for=condition=ready pod -l app=opentelemetry`)
	r.Run(`kubectl apply -k https://github.com/d-uzlov/deployments-k8s/examples/observability/jaeger-and-prometheus/nsm-system?ref=f3d8e97881a29f22b150bfcd6233647c5f96ee68`)
	r.Run(`WH=$(kubectl get pods -l app=admission-webhook-k8s -n nsm-system --template '{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}')` + "\n" + `kubectl wait --for=condition=ready --timeout=1m pod ${WH} -n nsm-system`)
	r.Run(`kubectl apply -k https://github.com/d-uzlov/deployments-k8s/examples/observability/jaeger-and-prometheus/example?ref=f3d8e97881a29f22b150bfcd6233647c5f96ee68`)
