...
func (s *Suite) TestJaeger_and_prometheus() {
	...
	r.Run(`kubectl apply -k https://github.com/d-uzlov/deployments-k8s/examples/observability/jaeger-and-prometheus?ref=4a0ec48044729b965bd0a6f161f5fbf2aaf5e0b4`)
	r.Run(`kubectl wait -n observability --timeout=1m --for=condition=ready pod -l app=opentelemetry`)
	r.Run(`kubectl apply -k https://github.com/d-uzlov/deployments-k8s/examples/observability/jaeger-and-prometheus/nsm-system?ref=4a0ec48044729b965bd0a6f161f5fbf2aaf5e0b4`)
	r.Run(`WH=$(kubectl get pods -l app=admission-webhook-k8s -n nsm-system --template '{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}')` + "\n" + `kubectl wait --for=condition=ready --timeout=1m pod ${WH} -n nsm-system`)
	r.Run(`kubectl apply -k https://github.com/d-uzlov/deployments-k8s/examples/observability/jaeger-and-prometheus/example?ref=4a0ec48044729b965bd0a6f161f5fbf2aaf5e0b4`)
